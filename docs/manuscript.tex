...
\[
  \kappa(T_{hyb})\lesssim \kappa(T_{add})\lesssim p^2 2^{m + M - 2\mathcal{M}}.
\]
...
The remaining preconditioner components -- application of matrices $\bm{R_C}$,
$\bm{R_C}^T$ and $\bm{A_R}$, and summation of the solvers' contributions
...
\begin{figure}
  ...
  for various $\mathcal{T}_\mathcal{H}$ and $\mathcal{T}_H$.
...
before reporting the results in Section~\ref{sec:amgx-comparison-results}.
