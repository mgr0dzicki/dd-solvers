...
We are now ready to present the algorithmic realization of the Schwarz preconditioners.
The setup phase, common to both preconditioners, is outlined in
Algorithm~\ref{preconditioner-setup-pseudocode}. Functions \textsc{M\_{add}}
and \textsc{M\_{hyb}\_{loc}}, which multiply the respective preconditioning matrices
...
Since all algorithms operate solely on matrix representations of the operators,
the boldface notation is omitted
for notational clarity. Further implementation details are provided in the next section.

\begin{algorithm}
  % When editing this algorithm, go through all references and check whether
  % the line numbers are correct.
  \caption{ASM Preconditioner -- Setup}
  ...
  \ParFor(\Comment*[f]{$A_C\gets R_CAR_C^T$}){$i,j=0, ..., N_{\mathcal{H}}-1$} {
    $(A_C)_{ij}\gets \Call{Sum}{A[k\bar l_{i} \slice k\bar l_{i + 1} - 1][k\bar l_{j} \slice k\bar l_{j + 1}-1]}$  \Comment*[r]{\eqref{A_C_structure}}
  }
...
\begin{algorithm}
  % When editing this algorithm, go through all references and check whether
  % the line numbers are correct.
  \caption{ASM Preconditioner -- Multiplication by a vector}
  \label{preconditioner-M-pseudocode}
  \KwReq{$k$, $l$, $\bar l$, \textsc{CoarseSolve}, \textsc{LocalSolve}$[0\slice N_H-1]$}
  \Function(\Comment*[f]{computes $Mx$, where $M$ is as in \eqref{M_matrix}}){M($x$)} {
    \ParFor(\Comment*[f]{$x^C\gets R_C x$}){$i=0, ..., N_{\mathcal{H}}-1$} {
      $x^{C}_i\gets \Call{Sum}{x[k\bar l_{i} \slice k \bar l_{i + 1}-1]}$  \Comment*[r]{\eqref{R_C_effect}}
    }
    \ParFor{$i=0, ..., N_H-1$} {
      $x^{(i)}\gets x[ki \slice k(i + 1)-1]$  \Comment*[r]{$x^{(i)}\gets R_ix$ \eqref{R_i_effect}}
    }
    $y^{C}\gets \Call{CoarseSolve}{x^{C}}$  \Comment*[r]{$y^{C}\gets A_C^{-1}x^{C}$}
    \ParFor{$i=0, ..., N_H-1$} {
      $y^{(i)}\gets \textsc{LocalSolve}[i]\!\left(x^{(i)}\right)$  \Comment*[r]{$y^{(i)}\gets A_i^{-1}x^{(i)}$}
    }
    \ParFor(\Comment*[f]{$y \gets R_0^Ty^{(0)}+ ...+R_{N_H-1}^Ty^{(N_H-1)}$}){$i=0, ..., N_H-1$} {
      $y[ki \slice k(i + 1) - 1] \gets y^{(i)}$  \Comment*[r]{\eqref{R_i_T_effect}}
    }
...
This chapter describes our GPU implementation of the ASM solver for the linear
system~\eqref{auf}. The implementation targets NVIDIA GPUs with compute capability 8.0 or
higher, but we expect that the conclusions we draw from its evaluation
in Chapter~\ref{chap:experiments} generalize to other highly parallel architectures.

The code is included in the \texttt{dd\_solvers} package
attached to this thesis. It addition to the ASM solver itself, it
provides solvers suitable for its coarse and local components, as well as
other methods used for preprocessing and performance comparison.

We begin by outlining the implementation environment in
Section~\ref{sec:hardware-and-software}, then review the dependencies
among the project's components in Section~\ref{sec:solvers-overview}.
Sections~\ref{sec:asm-preconditioner} and~\ref{sec:explicit-inverse-batch-solver}
detail the implementations of the ASM preconditioner and one of the possible
choices for its local component -- the explicit inverse batch solver. Next,
Section~\ref{sec:cg-method} presents an optimized implementation of the
(Preconditioned) CG method. Finally, Section~\ref{sec:amgx-preconditioner}
describes the configuration of the AmgX solver, which serves as our performance
baseline.
...
If PyTorch operations prove insufficient, we implement custom CUDA kernels.
Below we briefly summarize the CUDA programming model and highlight the
aspects of PyTorch that are most relevant to our implementation.

\subsubsection{CUDA Programming Model}
\label{sec:gpu-programming-model}

The architecture of Graphics Processing Unit (GPU) differs substantially from
that of Central Processing Unit (CPU). While the CPU is designed to
efficiently execute a sequence of operations, called a \emph{thread},
and can run at most tens of complex threads concurrently, the GPU is optimized for
executing thousands of threads in parallel, each performing a relatively
simple task (see Figure~\ref{fig:gpu-vs-cpu}).
Therefore to exploit the GPU's peak instruction throughput
and memory bandwidth, which can exceed those of the CPU by orders of magnitude
for comparable hardware cost, one needs algorithms that are \emph{massively parallel}:
able to split the work into thousands or even millions of inexpensive, independent tasks
\cite[Chapter~3]{cuda-guide}.

% \begin{figure}
%   \centering
%   \includegraphics[width=0.8\textwidth]{gpu_vs_cpu}
%   \caption{Comparison of CPU and GPU architectures. Individual \emph{cores} are
%     single-threaded units; accesses to the global memory (\emph{DRAM})
%     are buffered in smaller, but much faster \emph{caches}. The GPU
%     devotes more transistors to data processing rather than data caching and
%     control logic. Source: \cite[Figure 1]{cuda-guide}.}
%   \label{fig:gpu-vs-cpu}
% \end{figure}

Because our implementation targets NVIDIA hardware, it follows the CUDA
programming model \cite[Chapter 5]{cuda-guide}. In CUDA
the basic unit of execution is a \emph{kernel}: a function
executed in parallel by many GPU threads. Kernels are typically launched from
the CPU and execute asynchronously, allowing the CPU to launch kernels
without waiting for each to finish and thus keeping the GPU busy.
The GPU is usually equipped with its own, high-bandwidth memory; data must be
explicitly copied from the CPU to the GPU to be accessible to kernels,
and vice versa. The typical workflow therefore consists of copying input data to the GPU,
launching a series of kernels to process it, and finally copying the results back
to the CPU.

Each kernel contains code executed by a single GPU thread; these threads form blocks and blocks
form a~grid. For the programmer's convenience, this grid
might be indexed with one-, two- or three-dimensional coordinates. Because we use
a one-dimensional distribution of work (such as one matrix
row per thread) in all parallel algorithms presented in
this work, our kernels naturally employ 1D grids and blocks. In the
pseudocode, we use $thread\_global\_idx$ to denote the unique index of a
thread in the grid, which in the CUDA model is expressed as
\[
  thread\_global\_idx = blockI\hspace{-1pt}dx.x \cdot blockDim.x + threadI\hspace{-1pt}dx.x,
\]
where $blockI\hspace{-1pt}dx$, $blockDim$ and $threadI\hspace{-1pt}dx$ denote
the coordinates of the block in the grid, the dimensions of the block and the coordinates
of the thread within the block, respectively.

The native language for writing CUDA kernels is C++ with some extensions,
commonly referred to as CUDA C++ \cite[Chapter 6]{cuda-guide}. We use it to
...
This choice simplifies integration with the PyTorch framework,
which is used to implement the large part of the solver.
Moreover, static parameters of the kernels,
such as data types or the BSR block size, do
need to be specified in advance, but can be inferred from the input tensors.
Kernels are compiled on first invocation and then cached for all subsequent calls.

\subsubsection{PyTorch}
\label{sec:pytorch}

PyTorch 2 \cite{pytorch,pytorch2} offers a convenient interface for
linear algebra operations on both dense and sparse matrices and supports
a variety of hardware backends, including CPUs, GPUs and TPUs. In particular,
it can efficiently delegate certain operations to NVIDIA
CUDA libraries, such as cuBLAS \cite{cublas} and cuSPARSE \cite{cusparse},
which are highly optimized for NVIDIA GPUs. This makes PyTorch a suitable
choice for our needs, even though it is primarily designed
for applications in machine learning.

Installing PyTorch with all dependencies required for optimal performance
can be a complicated process and therefore it is recommended to use
the official Docker container images released by NVIDIA \cite{pytorch-ngc}.
The code presented in this work was tested on the container tagged as
\texttt{nvcr.io/nvidia/pytorch:24.12-py3}, which includes Python 3.12.3,
PyTorch 2.6.0 and NVIDIA CUDA 12.6.3. All additional solver dependencies
are specified in the accompanying \texttt{Dockerfile}.

The basic data structure in PyTorch is a \emph{tensor}, which is a
multidimensional array with an associated data type. The tuple of its
dimensions is usually referred to as its \emph{shape}. For example, a tensor
of shape $(m, k, k)$ is a three-dimensional array that can be viewed as a~batch
of $m$ square matrices of size $k\times k$, whereas a tensor
of shape $(m,)$ is a one-dimensional vector of length $m$.

PyTorch also supports a variety of sparse formats
designed to efficiently store tensors with a large number of zero elements.
The described solver operates on two of them:
\emph{Compressed Sparse Row} (CSR) and \emph{Block Sparse Row} (BSR).
For a detailed description of these formats,
see \cite[Section 3.3]{cusparse}.

By default, PyTorch performs tensor operations \emph{eagerly}, executing them
immediately and often creating extraneous intermediate tensors, which may lead
to significant performance degradation (see Sections~\ref{sec:preconditioner-solve}
and \ref{sec:fused-operations} for some examples).
PyTorch 2 addresses this issue via TorchDynamo and TorchInductor~\cite{pytorch2}.
TorchDynamo analyses Python code, bulding a~computation graph, which is then
just-in-time compiled by one of the available backends, TorchInductor being the
default one. Utilizing these tools is as simple as
extracting the code to be optimized into a function and decorating it with
\verb|torch.compile|, see, e.g., Section~\ref{sec:preconditioner-solve}.
...
Figure~\ref{fig:solvers-uml}. It defines three abstract classes -- \verb|SparseSolver|,
\verb|DenseBatchSolver| and \verb|SparseBatchSolver| -- which specify the
...
At the core of the package is \texttt{ASM}, in which we implement the preconditioner
described in Algorithms~\ref{preconditioner-setup-pseudocode}
and~\ref{preconditioner-M-pseudocode}. The details are discussed
in Section~\ref{sec:asm-preconditioner}. \texttt{ASM} is parametrized by
...
Since \texttt{ASM} is a preconditioner, it must
be used in conjunction with an iterative method. Following Section~\ref{sec:additive-schwarz-method}
we employ the Preconditioned Conjugate Gradient method \cite[Chapter~9.2]{Saad}.
As it is not provided by either PyTorch or cuSPARSE, we implement it ourselves
in the \texttt{CG} class. Any \verb|SparseSolver| can serve as
a~preconditioner; setting it to \verb|None| (default) yields an unpreconditioned CG solver.
...
the NVIDIA cuDSS library (v0.6.0)~\cite{cudss}, which
is a a high-performance direct sparse solver. As \verb|CUDSS| implements the \texttt{SparseSolver}
interface, it can be used either as the coarse solver within \texttt{ASM}
...
Available dense batch solvers for the \verb|ASM| local subproblems are
...
improvement. Accordingly, we supplement the ASM
preconditioner with two levels of precision control. First, the \texttt{precision}
attribute of the \texttt{ASM} class may be set to either \verb|float64| or
...
reduce the local solver's precision to 16 bits (\verb|bfloat16| or
\verb|float16|). We also allow the \verb|AMGX| preconditioner to run in
either single (\verb|float32|) or double (\verb|float64|) precision.
...
# Example 4: PCG with ASM preconditioner using sparse direct solvers
# for both coarse and local problems.
s4 = CG(ASM(
    coarse_solver=CUDSS(),
    local_solver=BatchCUDSS(),
))

# Example 5: PCG with ASM preconditioner applied in single precision,
# using dense local solver in a 16-bit precision.
s5 = CG(ASM(
    precision=torch.float32,
    coarse_solver=CUDSS(),
    local_solver=Inv(precision=torch.bfloat16),
))
\end{lstlisting}
\vspace{-3pt}
\end{algorithm}
...
\subsection{ASM Preconditioner}
\label{sec:asm-preconditioner}

We now describe the implementation of the ASM preconditioner,
outlined in Algorithms~\ref{preconditioner-setup-pseudocode}
and~\ref{preconditioner-M-pseudocode}.
The setup phase (see Algorithm~\ref{preconditioner-setup-pseudocode})
begins with constructing the coarse and local solvers' matrices, as detailed in Sections
\ref{sec:local-solvers-assembly} and \ref{sec:coarse-solver-assembly}. Next,
the coarse and local solvers are initialized using their \texttt{setup} methods.

The multiplication by $\bm{M}$ (see Algorithm~\ref{preconditioner-M-pseudocode})
-- which can be interpreted as an approximate solution of a system with $\bm{A}$
-- is implemented in the \texttt{solve} method of the \texttt{ASM}
class. Once the preconditioner is set up, it can be applied repeatedly to
different right-hand sides. Implementation details are provided in
Section~\ref{sec:preconditioner-solve}.

Note that the ASM preconditioner requires a specific ordering of the unknowns,
...
\subsubsection{Construction of the Local Solvers' Matrices}
\label{sec:local-solvers-assembly}

The procedure for constructing the matrices $\bm{A_0}, ..., \bm{A_{N_H-1}}$ depends
on the choice of the local solver, which might be either a \texttt{DenseBatchSolver}
or a \texttt{SparseBatchSolver} (see Section~\ref{sec:solvers-overview}). In
both cases, we develop tailored CUDA kernels. While expressing these constructions in
PyTorch is possible, it requires creating intermediate indexing tensors, which substantially
increase memory usage during setup and thus limit the maximum problem size that
can be handled. Our kernels overcome these limitations.

\paragraph{Dense case}

By our requirements, all local matrices $\bm{A_0}, ..., \bm{A_{N_H-1}}$ have identical dimensions
$k \times k$, where $k$ is the number of DoFs per local solver. Consequently, they can
be stored in a single tensor of shape $(N_H, k, k)$, which is the format expected
by \texttt{DenseBatchSolver} and enables efficient batched solutions.

\begin{algorithm}
  \caption{Construction of the local solvers' matrices -- dense case}
  \label{alg:assemble-local-matrices-dense}
  $row \gets thread\_global\_idx$\;
  \lIf {$row \geq A.crow\_indices.size - 1$} {\Return }
  $solver \gets \lfloor row\ /\ k\rfloor $\;
  $local\_row \gets row\!\!\mod k$\;
  \For {$i \gets A.crow\_indices[row];\; i < A.crow\_indices[row + 1];\; i\!+\!+$} {
    $col \gets A.col\_indices[i]$\;
    \uIf {$\lfloor col\ /\ k\rfloor = solver$} {
       $local\_col \gets col\!\!\mod k$\;
       $local\_mat[solver][local\_row][local\_col] \gets A.values[i]$\;
    } \uElseIf {$\lfloor col\ /\ k\rfloor > solver$} {
       \Return
    }
  }
\end{algorithm}
...
First, a tensor $local\_mat$ of shape $(N_H, k, k)$ is allocated on the GPU and filled with
zeros. Then, the kernel sketched in Algorithm~\ref{alg:assemble-local-matrices-dense}
iterates over the nonzero entries of the stiffness matrix $\bm{A}$, with each
thread responsible for a single row. If the processed
element has coordinates $(row, col)$ in~$\bm{A}$, it belongs
to the local solver's matrix $\bm{A_{solver}}$ precisely when
$solver = \lfloor row / k \rfloor = \lfloor col / k \rfloor$ (see \eqref{A_i_structure}).
In turn, its local coordinates are $(row\!\!\mod k, col\!\!\mod k)$. It
is assumed that the column indices in the CSR representation of $\bm{A}$
are sorted within each row\footnote{This assumption is formally included in the
CSR format definition. However, cuSPARSE allows a relaxed definition,
which only requires that the column
indices are unique within each row, not necessarily sorted \cite[Section~3.3.3]{cusparse}.}, and therefore the loop over the row's entries
can be terminated as soon as $\lfloor col / k \rfloor > solver$.

\paragraph{Sparse case}

If the local matrices are large enough, it may pay off to turn to a sparse local
solver, which is represented by the \texttt{SparseBatchSolver} class. In this case,
the local matrices are stored in a~single \texttt{CSRBatch} object, a
data structure defined in \texttt{dd\_solvers}.

A \texttt{CSRBatch} stores a batch of CSR matrices $\bm{A_0}, ..., \bm{A_{N-1}}$
as a tuple of five tensors:
\begin{itemize}
  \item \textit{nrows} -- a tensor of shape $(N,)$ containing the number of rows
    in each matrix $\bm{A_i}$,
  \item \textit{nnz} -- a tensor of shape $(N,)$ containing the number of nonzero
    entries in each matrix~$\bm{A_i}$,
  \item \textit{crow\_indices} -- a tensor of shape $(\sum_{i=0}^{N-1} (nrows[i] + 1),)$
    containg the concatenated row-offset arrays of all matrices~$\bm{A_i}$,
  \item \textit{col\_indices} -- a tensor of shape $(\sum_{i=0}^{N-1} nnz[i],)$
    containing the concatenated column index arrays of all matrices~$\bm{A_i}$,
  \item \textit{values} -- a tensor of shape $(\sum_{i=0}^{N-1} nnz[i],)$
    containing the concatenated value arrays of all matrices~$\bm{A_i}$.
\end{itemize}
Here $N=N_H$ and the $nrows$ tensor is filled with~$k$. The remaining
\texttt{CSRBatch} tensors are constructed in two stages.

First, we compute the tensor $per\_row\_count$ of shape $(n,)$,
where $per\_row\_count[r]$ is the number of nonzero entries in the $r$-th row of
$\bm{A}$ that belong to the local solver's matrix $\bm{A_{\lfloor r/k \rfloor}}$.
It is also the number of nonzero entries in the ${(r\!\!\mod k)}$\nobreakdash-th row
of $\bm{A_{\lfloor r/k \rfloor}}$. The $per\_row\_count$ tensor is computed
by the CUDA kernel which implements Algorithm~\ref{alg:assemble-local-matrices-sparse-i}.
Iteration over the elements of $\bm{A}$ and mapping them to the local
solvers' matrices is performed in the same way as in the dense case (cf.
Algorithm~\ref{alg:assemble-local-matrices-dense}).

\begin{algorithm}
  \caption{Construction of the local solvers' matrices -- sparse case, stage 1}
  \label{alg:assemble-local-matrices-sparse-i}
  $row \gets thread\_global\_idx$\;
  \lIf {$row \geq A.crow\_indices.size - 1$} {\Return }
  $solver \gets \lfloor row\ /\ k\rfloor $\;
  $count \gets 0$\;
  ...
  $per\_row\_count[row] \gets count$
\end{algorithm}

Once the $per\_row\_count$ array is available, the $nnz$ tensor is obtained
by summing the entries of $per\_row\_count$ for each local solver. Since the
number of DoFs per each local solver is $k$, it can be computed as
\begin{verbatim}
  nnz = per_row_count.reshape(-1, k).sum(dim=1).
\end{verbatim}

The same $per\_row\_count$ array is used to compute the $row\_of\!f\!set$ tensor,
which is a~cumulative sum of the $per\_row\_count$ entries, i.e.
\begin{equation}
row\_of\!f\!set[0] = 0,\quad row\_of\!f\!set[i] = \textstyle\sum_{j=0}^{i-1} per\_row\_count[j]
\quad \text{for }i=1,...,n.\label{eq:row-offset}
\end{equation}
In PyTorch, the parallel cumulative sum is efficiently implemented via
the \verb|cumsum| function. Note that $row\_of\!f\!set[n]$ equals
the total number of nonzero entries across all local solvers' matrices, which determines
the lengths of the $col\_indices$ and $values$ tensors. We then
allocate those tensors together with $crow\_indices$ of
shape $(N_H \cdot (k + 1),)$.

In the second stage, the CUDA kernel implementing Algorithm~\ref{alg:assemble-local-matrices-sparse-ii}
fills $crow\_indices$, $col\_indices$, and $values$.
Elements in the row $r$ of $\bm{A}$ that should be placed in one of the local solvers'
matrices are identified in the same way as before. Their values and column indices
are written in the corresponding tensors at positions
$row\_of\!f\!set[r], ..., row\_of\!f\!set[{r + 1}] - 1$. Finally, the $crow\_indices$
entries are computed from $row\_of\!f\!set$ via
\[
crow\_indices[s \cdot (k + 1) + l] = row\_of\!f\!set[s \cdot k + l] - row\_of\!f\!set[s \cdot k],
\]
for $s = 0, ..., N_H - 1$ and $l = 0, ..., k$.

\begin{algorithm}
  \caption{Construction of the local solvers' matrices -- sparse case, stage 2}
  \label{alg:assemble-local-matrices-sparse-ii}
  $row \gets thread\_global\_idx$\;
  \lIf {$row \geq A.crow\_indices.size - 1$} {\Return}
  $solver \gets \lfloor row\ /\ k\rfloor $\;
  $local\_row \gets row\!\!\mod k$\;
  $out\_ind \gets row\_of\!f\!set[row]$\;
  $first\_solver\_ind \gets row\_of\!f\!set[solver \cdot k]$\;
  \uIf {$local\_row = 0$} {
    $local\_crow\_indices[solver \cdot (k + 1)] \gets out\_ind - first\_solver\_ind$
  }
  \For {$i \gets A.crow\_indices[row];\; i < A.crow\_indices[row + 1];\; i\!+\!+$} {
    $col \gets A.col\_indices[i]$\;
    \uIf {$\lfloor col\ /\ k\rfloor = solver$} {
      $local\_col \gets col\!\!\mod k$\;
      $local\_values[out\_ind] \gets A.values[i]$\;
      $local\_col\_indices[out\_ind] \gets local\_col$\;
      $out\_ind\!+\!+$
    } \uElseIf {$\lfloor col\ /\ k\rfloor > solver$} {
      \Break
    }
  }
  $local\_crow\_indices[solver \cdot (k + 1) + local\_row + 1] \gets out\_ind - first\_solver\_ind$
\end{algorithm}

\subsubsection{Construction of the Coarse Solver's Matrix}
\label{sec:coarse-solver-assembly}

Construction of the coarse solver's matrix $\bm{A_C}$ (lines 5--7 of Algorithm
\ref{preconditioner-setup-pseudocode}) first assembles the matrix in PyTorch's
Coordinate (COO) format for sparse tensors \cite[Sparse COO tensors]{torch-sparse},
and then converts it to CSR format expected by \verb|SparseSolver|.

In COO format,
a~sparse 2D matrix with $m$ nonzero entries is represented by two tensors:
\begin{itemize}
  \item $indices$ -- a tensor of shape $(2, m)$, where the first row contains
    the row indices and the second row contains the column indices of the nonzero
    entries;
  \item $values$ -- a tensor of shape $(m,)$ containing the corresponding
    values of the nonzero entries.
\end{itemize}
PyTorch permits \emph{uncoalesced} COO tensors, in which $indices$ may include
duplicates; the value at a given coordinate is then the sum of all duplicated entries.
Converting such tensor to CSR requires \emph{coalescing}, which groups duplicate
indices and sums their values.

Let $solvers\_to\_coarse$ be a tensor of shape $(N_H,)$ such that
$solvers\_to\_coarse[j]=i$ whenever $\Omega_j \subseteq D_i$ for
$j=0, ..., N_H - 1$. One can compute it via
\begin{verbatim}
  torch.arange(n_coarse).repeat_interleave(solvers_per_coarse),
\end{verbatim}
where $n\_coarse$ is equal to $N_{\mathcal{H}}$ and $solvers\_per\_coarse$
is a tensor of shape $(N_H,)$ containing the values $l_0, ..., l_{N_H - 1}$ (see \eqref{l_i_def}).
From \eqref{A_C_structure}, an entry $(r,c)$ of the stiffness matrix $\bm{A}$
contributes to entry $(r',c')$ of $\bm{A_C}$, where
\begin{equation}
  r' = solvers\_to\_coarse[\lfloor r / k \rfloor],\quad
  c' = solvers\_to\_coarse[\lfloor c / k \rfloor].\label{rcr'c'}
\end{equation}
Thus, one can assemble $\bm{A_C}$ by creating an uncoalesced COO tensor of all
nonzeros of $\bm{A}$ with indices mapped according to the above, then coalescing
and converting to CSR.
However, the coalescing step involves sorting the $indices$ tensor, which
requires significant amount of additional memory. To mitigate this, we use the fact that
elements from each row of $\bm{A}$ that map to the same entry of $\bm{A_C}$
are contiguous in the CSR representation. We therefore group them during COO assembly;
only contributions from different rows must be merged in the final coalescing step,
reducing the additional memory required.

The intermediate COO tensor is constructed in two stages. First, the
CUDA kernel which implements Algorithm~\ref{alg:assemble-coarse-matrix-i} computes the tensor
$per\_row\_count$ of shape $(n,)$. Here $per\_row\_count[r]$ is the number of
entries in the COO tensor resulting from the reduction of the $r$-th row of $\bm{A}$.
The kernel runs on a 1D CUDA grid, where each thread handles one
row of $\bm{A}$. The mapping defined by \eqref{rcr'c'} determines which entries are summed.

\begin{algorithm}
  \caption{Construction of the coarse solver's matrix -- stage 1}
  \label{alg:assemble-coarse-matrix-i}
  $row \gets thread\_global\_idx$\;
  \lIf {$row \geq A.crow\_indices.size - 1$} { \Return }
  $count \gets 0$\;
  $prev\_block \gets -1$\;
  \For {$i \gets A.crow\_indices[row];\; i < A.crow\_indices[row + 1];\; i\!+\!+$} {
    $col \gets A.col\_indices[i]$\;
    $col\_block \gets solvers\_to\_coarse[\lfloor col\ /\ k\rfloor]$\;
    \uIf {$col\_block \neq prev\_block$} {
      $count\!+\!+$\;
      $prev\_block \gets col\_block$
    }
  }
  $per\_row\_count[row] \gets count$
\end{algorithm}

Next, the tensor $row\_of\!f\!set$ is computed as in \eqref{eq:row-offset}. Its
final entry, $row\_of\!f\!set[n]$, equals the total number of COO entries and
thus defines the lengths of $out\_values$ and $out\_indices$ tensors. In the
second stage, the CUDA kernel realizing Algorithm~\ref{alg:assemble-coarse-matrix-ii}
fills these tensors. Entries of row $r$
of $\bm{A}$ are grouped as before, their values are summed, and the result is
written to $out\_indices$ and $out\_values$ at positions
$row\_of\!f\!set[r], ..., row\_of\!f\!set[r+1]-1$.

\begin{algorithm}
  \caption{Construction of the coarse solver's matrix -- stage 2}
  \label{alg:assemble-coarse-matrix-ii}
  $row \gets thread\_global\_idx$\;
  \lIf {$row \geq A.crow\_indices.size - 1$} { \Return }
  $row\_block \gets solvers\_to\_coarse[\lfloor row\ /\ k\rfloor]$\;
  $out\_ind \gets row\_of\!f\!set[row]$\;
  $block\_sum \gets 0$\;
  $prev\_block \gets -1$\;
  \For {$i \gets A.crow\_indices[row];\; i < A.crow\_indices[row + 1];\; i\!+\!+$} {
    ...
    \uIf {$col\_block \neq prev\_block$} {
      \uIf {$prev\_block \neq -1$} {
          $out\_indices[0, out\_ind] \gets row\_block$\;
          $out\_indices[1, out\_ind] \gets prev\_block$\;
          $out\_values[out\_ind] \gets block\_sum$\;
          $out\_ind\!+\!+$\;
          $block\_sum \gets 0$
      }
      $prev\_block \gets col\_block$
    }
    $block\_sum\ +\!= A.values[i]$
  }
...
Finally, the COO tensor formed by $out\_indices$ and
$out\_values$ is coalesced and converted to the CSR format, yielding
the CSR representation of the coarse solver's matrix $\bm{A_C}$.

\subsubsection{Multiplication by a Vector}
\label{sec:preconditioner-solve}

The implementation of computing the preconditioner's action on a vector
is concise enough to appear in its entirety in Listing~\ref{lst:preconditioner-solve}.
Only metadata handling is omitted for clarity.
...
@torch.compile
def solve(self, x: torch.Tensor) -> torch.Tensor:                # $x\mapsto Mx$
  ...
  y += y_solvers.repeat_interleave(                          # $\hspace{4pt} y\ +\!= R_C^T y^C$
...
First, the $n$-element input vector $x$ is converted to lower precision if it is enabled
in the preconditioner configuration. Next, the vector $x^C=R_C x$ (cf.
Algorithm~\ref{preconditioner-M-pseudocode}, lines 2\nobreakdash--4) is computed and stored
in the tensor \verb|x_c|. Since this step sums consecutive blocks of
$kl_0, ..., kl_{N_H-1}$ elements of $x$, it can be split into two stages.
First, consecutive blocks of
$k$ elements of $x$ are summed (line 4) to form \verb|x_solvers| of
shape $(N_H,)$. Then, consecutive blocks of $l_0, ..., l_{N_H-1}$ elements of
\verb|x_solvers| are summed (lines 5--7) to produce \verb|x_c| of shape
...
Then, the two-dimensional tensor \verb|x_i| of shape $(N_H, k)$ is defined to
represent the batch of vectors $x^{(i)}=R_i x$
(cf. Algorithm~\ref{preconditioner-M-pseudocode}, lines 5--7). Since
vectors $x^{(i)}$ comprise blocks of $k$ consecutive elements of $x$, their
batched representation is obtained by simply \emph{reshaping}
the tensor $x$. In other words, \verb|x_i| shares memory with
\verb|x_lower_precision| and only indexes it differently; therefore no
GPU computation is invoked in this step.

Once \verb|x_c| and \verb|x_i| are available, the coarse and local
solvers are invoked in lines 8--11 (cf. Algorithm~\ref{preconditioner-M-pseudocode},
lines 8--11). The batch of outputs $y^{(i)}$ is
\emph{flattened} to form the vector $y$ of shape $(n, )$ in line 11
(cf. Algorithm~\ref{preconditioner-M-pseudocode}, lines 12--14). Again, this
requires no additional GPU computation, as flattening only changes the
interpretation of the indices.

Lines 12--17 compute $z=R_C^T y^C$ (cf. Algorithm~\ref{preconditioner-M-pseudocode}, line 15)
and add it to $y$ to yield the final result $y+z$.
Similarly to the reduction of $x$ to $x^C$, this uses a two-step construction
that we found to be more efficient than a single scatter operation.
First, the tensor \verb|y_solvers| of shape $(N_H,)$ is
constructed from \verb|y_c| by repeating the $i$-th element $l_i$ times. Then,
each element of \verb|y_solvers| is added to $k$ consecutive elements of
\verb|y|, yielding $y+z$.
The result is finally converted back to the original precision in line 18.

The entire method is decorated with \verb|torch.compile|
(see Section~\ref{sec:pytorch}), which fuses compatible
operations into a~single kernel. For example, downcasting in line 3 and
summation in line 4 are combined to eliminate a redundant read of
\verb|x_lower_precision|.

\subsection{Explicit Inverse Batch Solver}
\label{sec:explicit-inverse-batch-solver}

Using the explicit inverse as a local solver in the ASM preconditioner
may appear counterintuitive, since it may have worse numerical properties and requires a more
expensive setup than standard solvers based on factorizations such as LU or Cholesky \cite{Higham}.
...
Algorithm~\ref{preconditioner-M-pseudocode}, lines~9--11)
\[
...
the ASM preconditioner), which may be either double (\verb|float64|) or
single (\verb|float32|).
...
shape $(N, k)$. As a~solution, we propose a CUDA kernel that employs a single thread to compute each element
of the output tensor $y$, leading to the straightforward Algorithm~\ref{alg:batched-mat-vec}.
A major advantage of this approach is that it supports any matrix size $k$.

The matrices $mat$ are assumed to be
stored in column-major order (cf. line 7), which ensures that most accesses to the
...
\begin{algorithm}
  \caption{Batched dense matrix-vector product kernel}
  \label{alg:batched-mat-vec}
  $t \gets thread\_global\_idx$\;
...
Many combinations of $mat$, $x$, and $y$ are possible, but with the local solvers
use case in mind, we implemented four variants, listed in Table~\ref{tab:gemv-precisions}.
If the matrices $mat$ are stored in a 16-bit precision (\verb|float16| or \verb|bfloat16|),
their elements are converted to single precision immediately after being
fetched from memory (line 9). Thus multiplication
...
\begin{table}
  \centering
  \caption{Implemented variants of Algorithm \ref{alg:batched-mat-vec}.}
  \label{tab:gemv-precisions}
  \begin{tabular}{ccccc}
    \toprule
    $mat$ & $\times$ & $x$ & $=$ & $y$\\
    \midrule
    \verb|float64| & $\times$ & \verb|float64| & $=$ & \verb|float64|\\
    \verb|float32| & $\times$ &  \verb|float32| & $=$ & \verb|float32|\\
    \verb|float16| & $\times$ &  \verb|float32| & $=$ & \verb|float32|\\
    \verb|bfloat16| & $\times$ &  \verb|float32| & $=$ & \verb|float32|\\
    \bottomrule
  \end{tabular}
\end{table}
...
We benchmarked this metric on an A100 GPU for all supported precisions and for
$k=2,...,128$. For each $k$ we selected
the largest batch size $N$ satisfying $Nk^2\leq 2^{30}$
...
in mixed-precision cases we therefore downcast $x$ before
multiplication and upcast the result $y$ afterwards. This introduces extra
overhead and yields less accurate outputs than our implementation.

The results of the comparison are shown in Figure~\ref{fig:matrix_vec_tput}.
They indicate that our solution sustains high throughput
across the tested matrix sizes: $71-86\%$ of the A100 theoretical peak
memory bandwidth (according to the vendor datasheet)
for \verb|float64| and \verb|float32|, and $64-79\%$
for \verb|bfloat16| and \verb|float16|. Consequently, the theoretical upper
bound on the speedup any alternative algorithm could obtain (on an A100) is
approximately $1.16-1.56\times$, depending on precision and matrix size.
However, reaching that bound requires sustained peak memory\nobreakdash-bandwidth utilization,
which in practice is generally approached only by simple streaming operations
(for example, large contiguous device-to-device copies).
...
%   theoretical peak memory bandwidth of the A100 80GB SXM used in the experiments.}
%   \label{fig:matrix_vec_tput}
% \end{figure}

In contrast, the
PyTorch implementation performs poorly at the smallest matrix sizes, being
$10-60\times$ slower than ours for $k\leq 4$. Its throughput grows with $k$
and becomes competitive around $k\approx 64$, with the exact threshold
depending on the precision. However, the ASM preconditioner primarily targets
smaller matrix sizes when a~\verb|DenseBatchSolver| is used. In
Chapter~\ref{chap:experiments} we show that, in most scenarios, the optimal
choice is $21\leq k\leq 60$ with \verb|float16| precision -- settings for which
our implementation is $1.54 - 4.42\times$ faster than PyTorch.
Therefore we consistently employ our kernel in the \verb|Inv| class
for all precisions and matrix sizes.

\subsection{CG Method}
\label{sec:cg-method}

The implementation of the \texttt{CG} solver is based on the implementation
of \texttt{cg} function from \texttt{scipy.sparse.linalg} module \cite{scipy-cg},
with two important modifications which allow for better performance in our scenario:
fused operations and using BSR format for the matrix-vector product. Additionally,
to support the experiments in Section~\ref{sec:dependence-on-parameters},
we implemented an estimator for the condition number of the preconditioned
operator, computed from the PCG coefficients (see \cite[Section~6.7.3]{Saad}).

\subsubsection{Fused Operations}
\label{sec:fused-operations}

A naive PyTorch implementation of a single CG iteration is presented in
Listing~\ref{lst:cg-single-iter}. Because PyTorch operations are executed
eagerly, such implementation results in creation of unnecessary intermediate
tensors. For example, the execution of line 7
consists of launching two GPU kernels. The first one computes the right-hand
side \texttt{alpha * p} and stores it in the temporary tensor in the global memory.
The second kernel reads the result and adds it to \texttt{x}. The number of costly
memory operations can be reduced by fusing these two operations into a single
one.

\begin{algorithm}
\vspace{-3pt}
\begin{lstlisting}[
  caption={Simplified PyTorch implementation of a single PCG iteration.},
  label={lst:cg-single-iter},
  style=Python
]
...
This could have been realized by calling \verb|torch.add(x, p, alpha=alpha, out=x)|.
Unfortunately, the \verb|alpha| argument of \verb|torch.add| is currently
expected to be a CPU scalar. Therefore in our case \verb|alpha| would be
first transferred to the CPU, which would introduce an unnecessary
synchronization point.

However, it is not necessary to write a custom CUDA kernel to optimize this operation,
thanks to the \verb|torch.compile| feature described in Section~\ref{sec:pytorch}.
Applying it just to the expression in line 7 of
Listing~\ref{lst:cg-single-iter} yields an almost twofold speedup.
For example, on an NVIDIA A100 GPU with $x$ containing 100M elements,
the execution time of line 7 decreases from 1.17\,ms to 0.67\,ms.
Similar optimizations were applied to lines 4 and 8.

\subsubsection{BSR Matrix-Vector Product}

Looking at \ref{lst:cg-single-iter}, we see that most of the CG iteration
consists of simple element-wise vector operations like dot product, addition or
scalar multiplication. Therefore, the per-iteration cost is dominated by the preconditioner
solve (line 1) and the matrix-vector product (line 5). The latter can become
the most expensive step when using a lightweight preconditioner (such as massively
parallel ASM) as confirmed by our benchmarks.

We therefore optimize the computation of $\bm{A}\bm{p}$ by exploiting the block structure of the
stiffness matrix $\bm{A}$. If the unknowns associated with each element
...
four algorithms from \cite{bsr-mv}: block-by-block (b-by-b),
column-by-column (c-by-c), row\nobreakdash-per\nobreakdash-thread without synchronization (RPT) and
row-per-thread with synchronization (RPT-S). Reductions in the block-by-block and
column-by-column kernels use warp-shuffle operations \cite[Section 10.22]{cusparse}.
...
represented by the arrays $A.crow\_indices$,
$A.col\_indices$ and $A.values$. The algorithm computes the matrix-vector product
$Ax$ and stores the result in the $global\_out$ array. The RPT-I kernel likewise
executes on a one-dimensional grid but uses a~block size of 256.

\begin{algorithm}
  \caption{RPT-I multiplication kernel}
  \label{alg:bsr-mv-naive}
  $target\_row \gets thread\_global\_idx$\;
  ...
  \For{$block \gets first\_block;\; block < after\_last\_block;\; block\!+\!+$} {
    \For{$c\gets 0;\; c < b;\; c\!+\!+$} {
        $A\_elem \gets A.values[block][r][c]$\;
...
All variants were benchmarked on stiffness matrices for
several model problems of different sizes, with polynomial degress $p=1,...,5$ in 2D and $p=1,2,3$
in 3D case. The indices were stored as 32-bit integers in all cases, while the
data type of the matrix entries (and the whole computation) was either 32-bit
or 64-bit floating point. The time was measured on an NVIDIA A100 GPU and
an NVIDIA H100 GPU. Results appear in Table~\ref{tab:bsr-mv-benchmark}.

% \begin{table}
%   \caption{Speedup of our implementations of the BSR matrix-vector product over the
%   CSR matrix-vector product available in cuSPARSE. Block-by-block algorithm requires
%   $b\leq 5$.}
%   \label{tab:bsr-mv-benchmark}
%   \input{tables/benchmark_bsrmv.tex}
% \end{table}

The benchmarks demonstrate that the time of the matrix-vector product can be
significantly reduced by using BSR format, with the best speedups reaching
up to $2\times$ in single precision and $1.73\times$ in double precision.
While there is no algorithm that consistently outperforms CSR in every scenario,
the best overall results are achieved by the RPT-I algorithm.
It is only outperformed by CSR on small problems with $b=3$ and by RPT-S when
$b$ is large (15 or 21) in double precision. Both block-by-block and column-by\nobreakdash-column
perform poorly on our hardware, likely due to architectural
differences from the Kepler GPUs used in \cite{bsr-mv}. The cuSPARSE BSR routine
is competitive only at $b=4$, where RPT-I is still the fastest one. Note that
our comparison focuses on problems with more than three million unknowns; results
for smaller systems may differ.

Since the experiments in Chapter~\ref{chap:experiments} are run on
an A100 GPU and use double precision (fp64), we adopt the following heuristic
-- based on the benchmarks presented --
to select the BSR matrix\nobreakdash-vector product algorithm:
if $b<15$ use RPT-I, otherwise  use RPT-S.
...
If not stated otherwise, the right-hand side vector $\bm{f}$ is manufactured such that
...
with $p\geq 1$, which is not covered by either Theorem~\ref{thm:Przykry-asm}
or~\ref{thm:two-meshes-asm}, but was partially addressed in~\cite{Przykry-hp}.
In this setting we expect the condition number of the preconditioned operator
$T$ to satisfy
\[
  \kappa(T)\lesssim p^2\frac{\mathcal{H}^2}{hH}.
\]
...
The GPU we use to run all experiments in this chapter is an NVIDIA A100 80GB SXM.
We use CUDA Events \cite[Section 6.2.8.8]{cuda-guide} to measure GPU timings.
...
\subsection{Dependence on $h$, $H$, $\mathcal{H}$ and $p$}
\label{sec:dependence-on-parameters}

We begin by verifing the correctness of the implementation
by reproducing the experiment from \cite{Przykry}, which studies
the dependence of the condition number of the preconditioned ASM operator $\bm{T}$
on the mesh parameters in the massively parallel setting $\mathcal{T}_H=\mathcal{T}_h$.
For this test we set $d=2$, $p=1$, $\mathcal{T}_H=\mathcal{T}_h=\mathcal{S}_m$ and
$\mathcal{T}_\mathcal{H}=\mathcal{C}_\mathcal{M}$ for $4\leq \mathcal{M}\leq m\leq 11$.
Note that this extends the original experiment, which considered only
$4\leq \mathcal{M}\leq m\leq 7$.

All computations were performed in double precision, using an LU decomposition
as the local solver.
We measure the number of PCG iterations required to reduce the initial Euclidean
norm of the residual by a factor of $10^6$, and an estimate of the condition
number of $\bm{T}$ obtained from the PCG coefficients (see \cite[Section 6.7.3]{Saad}).
The corresponding solver configuration (cf. Section~\ref{sec:solvers-overview})
is given in Listing~\ref{lst:cond-experiments-solver}. The right-hand side
vector was generated by sampling uniformly from $[-1, 1]^{n}$ and normalizing
to unit Euclidean norm. The initial guess was taken to be the zero vector.

\begin{algorithm}
\vspace{-3pt}
\begin{lstlisting}[
  caption={Solver used to estimate the condition number of the preconditioned ASM operator.},
  label={lst:cond-experiments-solver},
  style=Python
]
CG(
...
Results are shown in Tables~\ref{tab:experiment-cond-h}
and~\ref{tab:experiment-cond-h-iters}. For $4\leq \mathcal{M}\leq m\leq 7$
the condition number estimates are close to those reported in \cite{Przykry},
although our values are slightly smaller \nobreakdash-- possibly because we employ
the exact form in the definition of the local solvers (see \eqref{T_i_def}).
For larger $m$ the estimates remain consistent and exhibit the
$O(\mathcal{H}^2/h^2)=O(2^{2m-2\mathcal{M}})$ behaviour, as predicted by
Theorem~\ref{thm:Przykry-asm} for $H=h$. The iteration counts follow the asymptotic behaviour
expected from Theorem~\ref{thm:pcg-convergence} given
the condition number estimates.
Surprisingly, our iteration counts are significantly
larger than those reported in~\cite{Przykry}; we currently have no explanation
for this discrepancy.

% \begin{table}
%   \centering
%   \renewcommand{\arraystretch}{1.15}
%   \caption{Dependence of the estimated condition number on the mesh parameters in the
%   massively parallel case:
%   $d=2$, $\mathcal{T}_H=\mathcal{T}_h=\mathcal{S}_m$ and
% $\mathcal{T}_\mathcal{H}=\mathcal{C}_\mathcal{M}$.}
%   \label{tab:experiment-cond-h}
%   \input{tables/experiment_cond_h.tex}
% \end{table}

% \begin{table}
%   \centering
%   \renewcommand{\arraystretch}{1.15}
%   \caption{Dependence of the PCG iteration count on the mesh parameters in the
%   massively parallel case:
%   $d=2$, $\mathcal{T}_H=\mathcal{T}_h=\mathcal{S}_m$ and
% $\mathcal{T}_\mathcal{H}=\mathcal{C}_\mathcal{M}$.}
%   \label{tab:experiment-cond-h-iters}
%   \input{tables/experiment_cond_h_iters.tex}
% \end{table}

Next, we verify the dependence on the polynomial degree $p$.
Again with $d=2$, we compare $p=1,2,...,12$ on the fixed mesh configuration:
$\mathcal{T}_h=\mathcal{S}_3$ and $\mathcal{T}_\mathcal{H}=\mathcal{T}_H=\mathcal{C}_3$,
using the same solver as above. The results are reported in Table~\ref{tab:experiment-cond-p}.
They clearly indicate $O(p^2)$ behaviour of the condition number for fixed
mesh parameters, which agrees with Theorem~\ref{thm:two-meshes-asm}, even
though the coefficient $\rho$ is not constant.

% \begin{table}
%   \centering
%   \renewcommand{\arraystretch}{1.15}
%   \caption{Dependence of the PCG iteration count and estimated condition number
%   on the polynomial degree $p$. Parameters: $d=2$, $\mathcal{T}_h=\mathcal{S}_3$ and
%   $\mathcal{T}_\mathcal{H}=\mathcal{T}_H=\mathcal{C}_3$.}
%   \label{tab:experiment-cond-p}
%   \input{tables/experiment_cond_p.tex}
% \end{table}
...
% \begin{table}
%   \centering
%   \caption{Dependence of the PCG iteration count on the preconditioner precision settings.
%   Parameters: $d=2$, $\mathcal{T}_h=\mathcal{S}_m$ and
%   $\mathcal{T}_H=\mathcal{T}_\mathcal{H}=\mathcal{S}_\mathcal{M}$. Entries
%   matching the double-precision results are in gray, extreme loss of
%   convergence is highlighted in red.}
%   \label{tab:experiment-precision-iters}
%   \input{tables/experiment_precision_iterations.tex}
% \end{table}
...
% \begin{figure}
%   \centering
%   \includesvg[width=0.65\textwidth]{precision_residuals}
...
\begin{table}
  ...
  \input{tables/experiment_speedups.tex}
