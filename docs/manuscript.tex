...
First, in line 3, \todo{w kodzie jest inna kolejnosc!!!}
...
The per-iteration cost of the PCG method is dominated by the preconditioner
solve and the matrix-vector product. The latter can become
the most expensive step when using a lightweight preconditioner (such as massively
parallel variant of the Schwarz preconditioner) as confirmed by our benchmarks.
...
or 64-bit floating point. The time was measured on an NVIDIA A100 GPU.
% Results appear in Table~\ref{tab:bsr-mv-benchmark}.

\begin{figure}
  \centering
  \includesvg[width=\textwidth]{bsrmv_memory_throughput}
  \caption{Performance of our implementations of the BSR matrix-vector
  product for various stiffness matrices. Block-by-block algorithm requires
  $b\leq 5$.}
