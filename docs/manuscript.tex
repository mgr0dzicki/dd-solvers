...
Due to the large number of parameters available in AmgX,
careful tuning is necessary to achieve optimal performance for a given problem
and, in our case, to ensure a "fair" comparison. For this reason, we provide
a detailed description of our setup of the AmgX preconditioner
...
parameters. The relative performance of the listed configurations was different
in 2D and 3D, and we expected more variation for larger $p$ and mixed-precision arithmetic.
For this reason, we retain all four configurations for the performance comparison
...
AmgX might be used to build a complete solver, as it provides implementations
of Krylov methods, including PCG. However, it does not support mixed-precision
computations and BSR matrix-vector product in combination with classical AMG.
Moreover, our primary interest is in comparing the preconditioners themselves,
rather than the implementations of the PCG method. Therefore we decided
to accelerate AmgX preconditioners using our PCG implementation.

To make sure this does not degrade AmgX performance, we...

\begin{figure}
  \centering
  \includesvg[width=0.8\textwidth]{amgx_pcg_throughput}
  \caption{Comparison of AmgX with different PCG variants.}
