...
\title{A Nonoverlapping Three-Grid, Two-Level Schwarz Method on a~Single GPU}
...
for examples). Following \cite{Przykry}, we decompose the finite element space $V_h$,
hereafter denoted simply by $V$, into the sum
\begin{equation}
...
$\mathcal{T}_H$ and $\mathcal{T}_{\mathcal{H}}$ are such that
...
\verb|float16|$\times$\verb|float64|) the difference is substantial.
...
Note that the restriction to the particular domain $\Omega$, mesh families
...
and the throughput of AmgX PCG and our equivalent variant (\verb|fp64|, CSR)
...
substantially faster, achieving speedups of up to $1.61\times$ when the best
configurations of both methods are compared.
...
Comparison with NVIDIA AmgX demonstrated that our ASM implementation is competitive:
